...
\title{From preferences to rewards: A brief introduction}
...
\begin{itemize}
  \item Explain when preference relations over trajectories admit ordinal utility representations, and what fails without completeness or transitivity.
  \item State and prove the von Neumann--Morgenstern theorem, and distinguish ordinal from cardinal utility.
  \item Derive the Markov reward representation from the temporal gamma-indifference axiom, and explain reward non-uniqueness and potential-based shaping.
\end{itemize}
...
Proposition~\ref{prop:ordinal_representation}, because any two real numbers are
...
obtained in Proposition~\ref{prop:ordinal_representation}: if preferences over
...
The general lesson is that reward is best understood as a \emph{coordinate
system} for utility, not as a uniquely given primitive. Some transformations,
...
\item Write down the Allais pattern from section 5 and explain which axiom it
violates.
\end{enumerate}
\end{exercise}
...
\item In the Allais pattern one prefers the sure prize in the first
comparison but the long-shot in the second, even though the two comparisons
differ only in a \emph{common consequence} added to both lotteries.
...
$u$ works. Otherwise $b\strictpref w$. Independence implies mixtures
$\alpha b+(1-\alpha)w$ are strictly increasing in $\alpha$. By continuity
the sets $\{\alpha: \alpha b+(1-\alpha)w \pref h\}$ and
$\{\alpha: h \pref \alpha b+(1-\alpha)w\}$ are closed; they are nonempty
($\alpha=1$, resp.\ $\alpha=0$, using (a)) and their union is $[0,1]$ by
completeness, so they intersect: some $u(h)$ lies in both, giving
$h\indiff u(h)b+(1-u(h))w$. Monotonicity makes $u(h)$ unique.
...
=a+b\,u(h),
\]
with $a=u'(w)$ and $b=u'(b)-u'(w)>0$ (positive because $b\strictpref w$ and
$u'$ represents $\pref$).
...
Assume the hypotheses of the previous exercise together with temporal
...
scaling and an additive shift modulated by $1-\gamma(t)$. Reward is a
coordinate system for utility, not a primitive of the preference.
\end{enumerate}
\end{solution}
...
safety-first preferences — violate continuity (see the earlier exercise);
