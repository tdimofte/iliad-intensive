...
[Appendix A](#a-solutions) provides worked solutions for all exercises.
...
1. **A linear neuron with bias** ([Example 1.1](#eg-neuron)). Let $\W = \R^{d+1}$, encoding a weight vector $w \in \R^{d}$ and a scalar bias $b \in \R$. The parameter–function map sends $(w, b)$ to $f_{w,b}: \R^{d} \to \R$ defined by

$$
f_{w,b}(x) = w^{\top} x + b.
$$
2. **Multi-linear neural network with bias** ([Example 1.2](#eg-linear)). Let $\W = \R^{m^2 + m}$, encoding a matrix $W \in \R^{m \times m}$ and a bias vector $b \in \R^{m}$. The parameter–function map sends $(W, b)$ to $f_{W,b}: \R^{m} \to \R^{m}$ defined by

$$
f_{W,b}(x) = Wx + b.
$$
3. **Deep linear network with bias** ([Example 1.3](#eg-dln)). Let $\W = \R^{2mh + h + m}$, encoding matrices $A \in \R^{h \times m}$, $B \in \R^{m \times h}$ and bias vectors $b_{A} \in \R^{h}$, $b_{B} \in \R^{m}$. The parameter–function map sends $(A, b_{A}, B, b_{B})$ to $f_{w} : \R^{m} \to \R^{m}$ defined by

$$
f_{w}(x) = B(Ax + b_{A}) + b_{B}.
$$
4. **Multi-layer perceptron with bias** ([Example 1.4](#eg-mlp)). The parameter space is $\W = \R^{2mh + h + m}$ as above. The parameter–function map sends $(A, b_{A}, B, b_{B})$ to $f_{w} : \R^{m} \to \R^{m}$ defined by
...
$$
3. From the mean squared error definition [(9)](#eq-mse),
...
-\log p(C_{n} \mid X_{n}, w) = -\sum_{i=1}^{n} \log f_{w}(c^{(i)}\mid x^{(i)}).
$$
3. Since $y^{(i)}= \delta_{c^{(i)}}$ is the Dirac distribution on class $c^{(i)}$, we have $y^{(i)}(j) = 1$ if $j = c^{(i)}$ and $y^{(i)}(j) = 0$ otherwise. Substituting into the cross entropy loss [(6)](#eq-cross-entropy),
...
Substituting into the definitions of the posterior [(13)](#eq-posterior) and partition function [(14)](#eq-partition-function) gives the stated expressions.
...
1. Integrating the posterior [(13)](#eq-posterior) over $\U$,
...
1. The hypothesis class of [Exercise 2.1](#ex-univariate-degeneracy) is $\F = \{f_{w} = w : w \in \R\} = \R$. The hypothesis class here is $\F = \{f_{w} = w^{3} : w \in \R\}$. Since $w \mapsto w^{3}$ is a bijection on $\R$, as $w$ ranges over $\R$, $w^{3}$ takes every real value exactly once. So $\F = \R$ in both cases.
...
1. The hypothesis class is $\F = \{f_{w} = ab : (a, b) \in \R^{2}\} = \R$, since for any target $c \in \R$ we can choose, e.g., $a = c$ and $b = 1$. This is the same as in [Exercise 2.1](#ex-univariate-degeneracy).
...
1. The hypothesis class is $\F = \{f_{w} = a + b : (a, b) \in \R^{2}\} = \R$, since for any target $c \in \R$ we can choose, e.g., $a = c$ and $b = 0$. This is the same as in [Exercise 2.1](#ex-univariate-degeneracy).
2. The directional derivative in direction $v = (1, -1)$ is
...
3. From [Exercise 2.3(d)](#ex-product-degeneracy), the coordinate directions $(1, 0)$ and $(0, 1)$ are degenerate precisely on the coordinate axes: $(1, 0)$ is degenerate on $\{b = 0\}$ and $(0, 1)$ is degenerate on $\{a = 0\}$. This matches exactly: the axis $\{b = 0\}$ is where the $a$-translation symmetry acts, providing the degenerate direction $(1, 0)$, and the axis $\{a = 0\}$ is where the $b$-translation symmetry acts, providing $(0, 1)$.
...
1. Since $v$ is a unit vector, $s_{v} = D_{v} \log p = \frac{v}{\|v\|}\cdot s = v \cdot s$. From the definition of $I(w)$ in [(26)](#eq-fim),
...
$$
2. If $D_{v} \Psi(w) = 0$ for some non-zero $v$, then by [Exercise 2.11(c)](#ex-score-properties), $s_{v}(x,y,w) = 0$ for all $x, y$. Let $\hat{v}= v / \|v\|$. Then $s_{\hat{v}}= s_{v}$ (the directional derivative is invariant to the magnitude of $v$), so by part (a), $\hat{v}^{\top} I(w)\, \hat{v}= \E[s_{\hat{v}}^{2}] = 0$. A matrix $M$ is positive definite only if $u^{\top} M u > 0$ for all non-zero $u$. Since $\hat{v}$ is a non-zero vector with $\hat{v}^{\top} I(w)\, \hat{v}= 0$, the matrix $I(w)$ is not positive definite.
3. If $I(w)$ is not positive definite, then since $I(w)$ is positive semidefinite, there exists a non-zero vector $\hat{v}$ such that $\hat{v}^{\top} I(w)\, \hat{v}= 0$. By part (a) (taking $\hat{v}$ to be a unit vector without loss of generality), $\E[s_{\hat{v}}^{2}] = 0$. Since $s_{\hat{v}}^{2} \geq 0$ and $q(x) > 0$ and $p(y \mid x, w) > 0$, this implies $s_{\hat{v}}(x,y,w) = 0$ for all $x \in \X$ and $y \in \Y$. By [Exercise 2.11(d)](#ex-score-properties), $D_{\hat{v}}\Psi(w) = 0$.
...
1. From [Exercise 2.11](#ex-score-properties), $\E_{y \sim p(y \mid x, w)}[s_{j}(x,y,w)] = 0$ for each component $j$ of the score, where $s_{j} = \frac{\partial}{\partial w_{j}}\log p(y \mid x, w)$. Differentiating with respect to $w_{k}$ and exchanging the derivative with the integral:
...
Using the score identity $\frac{\partial p}{\partial w_{k}}= p \cdot s_{k}$ [(24)](#eq-score-identity), the second integral becomes $\int_{\Y} s_{j}\, s_{k}\, p\, dy = \E_{y}[s_{j}\, s_{k}]$. Therefore
...
$$
\begin{aligned}H(w_{0})&= \E_{x \sim q(x)}\, \E_{y \sim p(y \mid x, w_0)}\!\bigl[ s(x,y,w_{0})\, s(x,y,w_{0})^{\top} \bigr] \\&= I(w_{0}). \quad\text{(by Definition 2.4)}\end{aligned}
$$
3. By [Exercise 2.12](#ex-fim-degeneracy), $\ker I(w_{0}) = \{v \in \R^{d} : D_{v} \Psi(w_{0}) = 0\}$. Since $H(w_{0}) = I(w_{0})$, if $\Psi$ is degenerate at $w_{0}$ in direction $v$, then $v \in \ker I(w_{0}) = \ker H(w_{0})$, so $H(w_{0})\, v = 0$.
...
1. The identity parameter–function map $\Phi(a,b) = (a,b)$ has Jacobian equal to the $2 \times 2$ identity matrix everywhere, so it is non-degenerate at every point. [Exercise 2.14(b)](#ex-loss-landscape-degeneracy) shows that $L_{1,1}$ has a non-degenerate minimum: absence of parameter–function map degeneracy is consistent with absence of loss landscape degeneracy. [Exercise 2.14(c)](#ex-loss-landscape-degeneracy) and [2.14(d)](#ex-loss-landscape-degeneracy) show that $L_{2,1}$, $L_{0,1}$, etc., have degenerate minima despite the parameter–function map being non-degenerate. This demonstrates that loss landscape degeneracy does not imply parameter–function map degeneracy.
...
3. Under the identity parameter–function map, $L_{1,1}(a,b) = a^{2} + b^{2}$ had a non-degenerate minimum at the origin ([Exercise 2.14(b)](#ex-loss-landscape-degeneracy)). Under the cubic parameter–function map, the same loss became $\alpha^{6} + \beta^{6}$, which has a degenerate minimum at the origin. This shows that introducing degeneracy into the parameter–function map can convert a non-degenerate minimum into a degenerate one.

For other $k,l$, $L_{k,l}$ was already degenerate at the origin under the non-degenerate parameterisation, the cubic function only makes it *more* degenerate (in terms of the order to which the loss vanishes in the degenerate direction(s); we will formally quantify this in [Section 3](#3-the-degeneracy-hierarchy)).
...
This does not contradict [Exercise 2.15](#ex-fim-hessian), which establishes $H(w_{0}) = I(w_{0})$ at the *true parameter* $w_{0}$, meaning $\Phi(w_{0})$ must equal the target. Here, $\Phi(0,0) = 0 \neq 1$, so $(0,0)$ is not the true parameter, and the identity does not apply.
...
1. From [Definition 3.1](#def-llc), as $\epsilon \to 0$,
...
2. From [Definition 3.1](#def-llc), $V(\epsilon) = c\,\epsilon^{\lambda(w^*)}(-\log\epsilon)^{\mu(w^*)-1}(1 + o(1))$, so
...
Since $\log C / \log \epsilon \to 0$ as $\epsilon \to 0^{+}$, taking the limit and using Equation [(39)](#eq-solve-llc) gives
...
Let $L_{1}(x,y) = x^{2}+y^{4}$ with $w_{1} = (0,0)$. Then $\nabla^{2}L_{1}(0,0) = \operatorname{diag}(2, 0)$, so $\operatorname{rank}\nabla^{2}L_{1}(0,0) = 1$. By [Exercise 3.4](#ex-calculating-llc), $\lambda(w_{1}) = \frac{3}{4}$.

Let $L_{2}(x,y) = x^{2}$ with $w_{2} = (0,0)$. Then $\nabla^{2}L_{2}(0,0) = \operatorname{diag}(2, 0)$, so $\operatorname{rank}\nabla^{2}L_{2}(0,0) = 1$. The minimiser locus is $\{x = 0\}$, which is the setting of [Exercise 3.2](#ex-quad-valley-scaling) with $d' = 1$, giving $\lambda(w_{2}) = \frac{1}{2}$.
...
The loss $L(w) = w_{1}^{2k_1}\cdots w_{d}^{2k_d}$ is already in monomial form, so the identity map $g = \mathrm{id}$ serves as a (trivial) resolution of singularities with $dw = 1 \cdot du$, i.e. $h_{j} = 0$ for all $j$. Substituting into [(43)](#eq-rlct) gives
...
Since $w^{*}$ is a local minimum and $L(w) \geq L(w^{*})$ on $B(w^{*})$, this simplifies to $B_{\epsilon}(w^{*}) = \{w \in B(w^{*}) : L(w) - L(w^{*}) < \epsilon\} = B(w^{*},\epsilon)$, the sublevel set from [(34)](#eq-sublevel-set). In particular, $\nu(B_{\epsilon}(w^{*})) = V(\epsilon)$ where $\nu$ is Lebesgue measure.
...
1. We first check which case of [Theorem 3.4](#thm-aoyagi-watanabe) applies. Setting $M = N = H = m$, the three conditions of Case 1 each reduce to $m + r \leq 2m$, i.e. $r \leq m$, which always holds. So we are always in Case 1. The parity condition is on $M + H + N + r = 3m + r$, which has the same parity as $m + r$.
...
so $\lambda$ is increasing in $r$ therefore **lower rank $\implies$ lower $\lambda$**. This is consistent with [Exercise 2.9(d)](#ex-dln-degeneracy) where we saw that at lower-rank parameters, more directions in parameter space are degenerate.
...
1. For $\mu_{0} = 0$, the LLC is $\lambda = 1/6$ ([Exercise 3.7](#ex-cubic-parameterisation)). Plot $F_{n}$ against $n$ and compare with the asymptotic estimate $\hat F_{n} = n L_{n}(0) + \tfrac{1}{6}\log n$. For large $n$, the two curves should agree up to an $O(1)$ offset: $F_{n} - \hat F_{n}$ should stabilise around a constant.
