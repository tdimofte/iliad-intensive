...
% summary: >-
%   Singular learning theory (SLT) places degeneracy as a core part of
%   understanding how neural networks learn. We cover the parameter-function
%   map, the meaning of degeneracy through the local learning coefficient, to
%   Watanabe's free energy formula and Bayesian phase transitions.
% slides: https://drive.google.com/drive/folders/1oO1gcMnfVTD56FACTq8pJI55ktapWXeQ
%--- end ---
...
% Matthew's recorded lecture for this day (public since ~2026-08). The title
% is fetched from YouTube's oEmbed endpoint at build time; while the video was
% still scheduled/private the lookup 403'd and the page shipped the "title
% unavailable" fallback. Beware for future sheets: the fetched title is NOT a
% build-cache input, so a sheet built before its premiere keeps the fallback
% on every rebuild ("cached — inputs unchanged") until some edit to the sheet
% busts its hash. This comment edit is that edit for this sheet.
\youtube{SzVmiN8BvCc}
...
\begin{enumerate}
    \item
        ...
        Read \cref{sec:degeneracy:hessian} and complete your choice of
            \cref{ex:loss-landscape-degeneracy} and/or \cref{ex:fim-hessian}.
    \item
...
\label{ex:biased-neurons}
    ...
    than just a linear transform. For each of
    \cref{eg:neuron,eg:linear,eg:dln,eg:mlp}, extend the example to use
...
distributions over image classes. Let $\C$ represent the underlying class
...
\begin{remark}[Negative log-likelihood loss]
    \Cref{ex:nll-mse,ex:nll-cross-entropy,ex:nll-energy} nominally show that some maximum likelihood estimation
...
where the integral in \eqref{eq:partition-function} is restricted to a certain
...
\begin{proof}
    ...
    Fix $t_0 \in \R$. Since $T_t$ is a continuous symmetry, the derivative
    along the curve at $t_0$ vanishes:
    \begin{align*}
        D_{\gamma'(t_0)} \Phi(\gamma(t_0))
        &\propto \gamma'(t_0) \cdot \nabla \Phi(\gamma(t_0))
    \\  &= \left.\frac{d}{dt}\right|_{t=t_0} \Phi(\gamma(t))
            \tag{chain rule}
    \\  &= \left.\frac{d}{dt}\right|_{t=t_0} \Phi(w)
...
The following is a guide to one possible approach.
        \begin{enumerate}
            ...
            \item 
                ...
                Show that the dimensionality of this image (the rank of the linear map $T_B$) is $m r_B$.
            \item 
        ...
        \end{enumerate}
        Alternatively: substitute singular value decompositions
        $A = U_1 \Sigma_1 V_1^\top$ and $B = U_2 \Sigma_2 V_2^\top$ into the
        degeneracy condition $B\,\delta\!A + \delta\!B\, A = 0$, and change
        variables to absorb the orthogonal factors. The condition becomes an
        equation between two diagonally-scaled matrices, whose solutions can
        be counted entry by entry.
    \end{hint}
\end{enumerate}
\end{exercise}
...
Differentiate the identity
    ...
    \item\label{ex:score-properties:c}
        Show the converse: if the directional score $s_v(x, y, w)$
...
\label{rem:strictly-singular}
    ...
    \citet{Watanabe2007,Watanabe2009} observe that many of the results from
...
\begin{enumerate}
    \item
        ...
        $D_v L(w) = 0$.
    \item
        Suppose $\W = \R^d$. Show that if $d > 1$, then for \emph{any} $w \in \W$, there exists a nonzero $v$
...
Note that the Hessian degeneracy in \cref{ex:lld:2,ex:lld:3} arises for
    ...
    \item
        ...
        \begin{enumerate}
            ...
            \item
                ...
                defined on the constant function $(a, b)$ (rather than the equivalent
...
\begin{exercise}[Quadratic Valley Volume Scaling]
    ...
    \begin{enumerate}
        ...
        \begin{hint}
Use the volume of the ellipsoid from \cref{ex:llc-regular-case}.
        \end{hint}
    \end{enumerate}
\end{exercise}

In this example, the degenerate directions do not contribute to the leading exponent of $V(\epsilon)$. We see that the exponent $\frac{d}{2}$ from \cref{ex:llc-regular-case} is replaced with $\frac{d'}{2}$ where $d'$ is the codimension of $\W_0$ i.e. the \textit{effective dimension} of $L$ which ignores the degenerate directions. This suggests that we can capture information about the degree of degeneracy of a loss function by looking at how the volume $V(\epsilon)$ scales as $\epsilon \to 0$. \cite{Watanabe2009} proves a general form for this kind of volume scaling which has been adapted by \cite{Lau+2025} into the following definition.
...
\label{ex:LLC-hessian-rank}
    In this exercise we will show that the LLC can detect differences in the local geometry that are not reflected in the rank of the Hessian.
...
\citet{Chen+Murfet2025} characterise the sensitivity of local learning coefficient estimation to patterns in sequence models.
...
\citet{Elliott+2026} extend this study to a case of goal misgeneralisation in deep reinforcement learning.
...
\begin{enumerate}
    ...
    \item The direction $v = (1, -1)$ has $\|v\| = \sqrt{2}$, so by
    \eqref{eq:directional-derivative} the directional derivative is
    \begin{equation*}
        \frac{1}{\sqrt{2}}\left(
            1 \cdot \frac{\partial}{\partial a}(a + b)
            + (-1) \cdot \frac{\partial}{\partial b}(a + b)
        \right)
        = \frac{1}{\sqrt{2}}(1 - 1) = 0.
    \end{equation*}
...
\paragraph{Alternative solution to (d), via the SVD.}
Take singular value decompositions
    $A = U_1 \Sigma_1 V_1^\top$
and
    $B = U_2 \Sigma_2 V_2^\top$,
where $U_1, V_1, U_2, V_2 \in \R^{m \times m}$ are orthogonal and
$\Sigma_1, \Sigma_2 \in \R^{m \times m}$ are diagonal with non-negative
entries, ordered so that the $r_A$ non-zero entries of $\Sigma_1$ and the
$r_B$ non-zero entries of $\Sigma_2$ come first.
By part~(a), $(\delta\!A, \delta\!B)$ is a degenerate direction if and only
if $B\,\delta\!A + \delta\!B\, A = 0$.
Change variables by the linear map
\begin{equation*}
    X = V_2^\top\, \delta\!A\, V_1,
    \qquad
    Y = -U_2^\top\, \delta\!B\, U_1,
\end{equation*}
so that $\delta\!A = V_2 X V_1^\top$ and $\delta\!B = -U_2 Y U_1^\top$.
This map is invertible, so it preserves the dimensionality of subspaces,
and we may count degrees of freedom in the new variables. Substituting,
\begin{align*}
    B\,\delta\!A + \delta\!B\, A = 0
    &\iff
        U_2 \Sigma_2 V_2^\top\, V_2 X V_1^\top
        = U_2 Y U_1^\top\, U_1 \Sigma_1 V_1^\top
\\  &\iff
        U_2 \left( \Sigma_2 X \right) V_1^\top
        = U_2 \left( Y \Sigma_1 \right) V_1^\top
\\  &\iff
        \Sigma_2 X = Y \Sigma_1,
\end{align*}
using orthogonality ($V_2^\top V_2 = U_1^\top U_1 = I$) in the second step
and invertibility of $U_2$ and $V_1$ in the third.

It remains to count the dimensionality of the space of pairs $(X, Y)$
satisfying $\Sigma_2 X = Y \Sigma_1$.
Write $\sigma_i$ for the $i$-th diagonal entry of $\Sigma_2$ and $\tau_j$
for the $j$-th diagonal entry of $\Sigma_1$.
Multiplying by a diagonal matrix on the left scales rows, and on the right
scales columns, so entry $(i, j)$ of the equation reads
\begin{equation*}
    \sigma_i X_{ij} = \tau_j Y_{ij}.
\end{equation*}
Each such equation involves only the pair of entries $(X_{ij}, Y_{ij})$, and
each pair appears in exactly one equation, so the system decouples into
$m^2$ independent cells.
The picture is as follows: the bottom $m - r_B$ rows of $\Sigma_2 X$ and
the rightmost $m - r_A$ columns of $Y \Sigma_1$ vanish identically, so a
cell in their overlap yields the trivial equation $0 = 0$, while every
other cell yields one non-trivial constraint.
\begin{center}
\begin{tikzpicture}[line width=0.8pt]
  % geometry
  \def\m{4}      % matrix side
  \def\zB{1.2}   % height of zero rows  (m - r_B)
  \def\zA{1.2}   % width  of zero cols  (m - r_A)
  \def\gap{2.2}  % gap between matrices

  % ---- Left matrix: Sigma_2 X, bottom (m - r_B) rows zero ----
  \draw (0,0) rectangle (\m,\m);
  \fill[pattern=north east lines, pattern color=blue!60]
       (0,0) rectangle (\m,\zB);
  \draw[dashed, thin] (0,\zB) -- (\m,\zB);
  \node[above] at (\m/2, \m) {$\Sigma_2 X$};
  \draw[decorate, decoration={brace, amplitude=5pt}]
       (-0.1,0) -- (-0.1,\zB)
       node[midway, left=6pt] {$m-r_B$};
  \draw[->] (\m/2,-0.8) node[below] {$0$} -- (\m/2,\zB/2);

  % ---- Equals sign ----
  \node at ({\m + \gap/2}, \m/2) {\Huge $=$};

  % ---- Right matrix: Y Sigma_1, right (m - r_A) cols zero ----
  \begin{scope}[shift={({\m + \gap},0)}]
    \draw (0,0) rectangle (\m,\m);
    \fill[pattern=north east lines, pattern color=blue!60]
         (\m-\zA,0) rectangle (\m,\m);
    \draw[dashed, thin] (\m-\zA,0) -- (\m-\zA,\m);
    \node[below] at (\m/2, 0) {$Y \Sigma_1$};
    \draw[decorate, decoration={brace, amplitude=5pt}]
         (\m-\zA,\m+0.1) -- (\m,\m+0.1)
         node[midway, above=6pt] {$m-r_A$};
    \draw[->] (\m+0.9,\m/2) node[right] {$0$} -- (\m-\zA/2,\m/2);
  \end{scope}
\end{tikzpicture}
\end{center}
Concretely: the equation for cell $(i, j)$ is trivial if and only if
$\sigma_i = \tau_j = 0$, that is, $i > r_B$ and $j > r_A$, leaving both
entries free (two degrees of freedom); there are $(m - r_B)(m - r_A)$ such
cells. Every other cell imposes one non-trivial linear constraint on
$(X_{ij}, Y_{ij})$, leaving one degree of freedom; there are
$m^2 - (m - r_A)(m - r_B)$ such cells.
The dimensionality of the space of degenerate directions is therefore
\begin{align*}
    &2 (m - r_A)(m - r_B)
    + \left( m^2 - (m - r_A)(m - r_B) \right) \\
    &\quad= m^2 + (m - r_A)(m - r_B).
\end{align*}
\end{solution}
...
\begin{enumerate}
    ...
    \begin{align*}
        B(w^*, \epsilon) &= \left\{ (x_1,\dots,x_d) \in [-R,R]^d \;\middle|\; \sum_{i=1}^{d'} x_i^2 < \epsilon \right\} \\
        &= \left\{x \in \R^{d'} : \sum_{i=1}^{d'} x_i^2 < \epsilon \right\} \times [-R,R]^{d-d'}
    \end{align*}
...
\begin{enumerate}
    ...
    \item
        ...
        \begin{align*}
            V(\epsilon) &= \int_{-\epsilon^{1/4}}^{\epsilon^{1/4}} 2\sqrt{\epsilon - y^4}\, dy = 2\epsilon^{1/4} \int_{-1}^{1} \sqrt{\epsilon - \epsilon t^4}\, dt \\
            &= 2\epsilon^{3/4} \int_{-1}^{1} \sqrt{1 - t^4}\, dt = c\,\epsilon^{3/4},
        \end{align*}
